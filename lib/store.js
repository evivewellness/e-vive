...
// midnight (hours expressed on a 0–31 scale so day+1 arithmetic just works).
function shiftHourWindow(type) {
  if (type === 'night') return [19, 31];
  if (type === 'live-in' || type === '24-hour') return [0, 24];
  return [7, 19]; // day
}
function shiftInterval(dateStr, type) {
...
// DB queries total regardless of how long the range is.
export async function getHcaScheduleConflicts(hcaId, startDate, endDate, shiftType, { excludePlacementId } = {}) {
  const from = new Date(`${startDate}T00:00:00`); from.setDate(from.getDate() - 1);
  const to   = new Date(`${endDate}T00:00:00`);   to.setDate(to.getDate() + 1);
...
function describeConflicts(conflicts, clients = []) {
...
export async function createPlacement({ clientId, patientId, hcaId, startDate, endDate, shiftType = 'day', ratePerShift, notes = '' }) {
  ...
  const startTime = shiftType === 'night' ? '19:00' : (shiftType === 'live-in' || shiftType === '24-hour') ? '00:00' : '07:00';
...
export async function extendPlacement(id, newEndDate) {
  ...
  const addFrom = new Date(`${placement.endDate}T00:00:00`); addFrom.setDate(addFrom.getDate() + 1);
  const addFromStr = addFrom.toISOString().slice(0, 10);
  ...
  const startTime = placement.shiftType === 'night' ? '19:00' : (placement.shiftType === 'live-in' || placement.shiftType === '24-hour') ? '00:00' : '07:00';
  const titleName = patient?.name || client?.name || 'Patient';
...
export async function endPlacement(id, endDate = new Date().toISOString().slice(0, 10)) {
  await supabase.from('shifts').update({ status: 'cancelled' })
