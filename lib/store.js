...
export async function deleteHcaProfile(id) {
  ...
  // while those rows still exist. Detach/clean them up first.
  await supabase.from('shifts').delete().eq('hca_id', id);
  await supabase.from('cardex_entries').delete().eq('hca_id', id);
  await supabase.from('placements').delete().eq('hca_id', id);
  await supabase.from('calendar_events').delete().eq('hca_id', id);
  await supabase.from('payroll_payments').delete().eq('hca_id', id);
  await supabase.from('notifications').delete().eq('hca_id', id);
  await supabase.from('clients').update({ assigned_hca_id: null }).eq('assigned_hca_id', id);
  await supabase.from('clients').update({ requested_hca_id: null }).eq('requested_hca_id', id);
