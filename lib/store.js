...
import { isCoord, normaliseRadius } from './geofence';
...
// ─── ATTENDANCE ───────────────────────────────────────────────────────────────
// Clocking in and out goes through /api/shifts/clock, which resolves the shift
// and the patient's care address from the HCA's signed session cookie and
// decides the geofence server-side.
//
// This used to happen in the browser: the page computed the distance itself
// and then wrote the result with the public anon key. Anyone could spoof the
// position from developer tools, or call this function from the console with
// whatever coordinates they liked, and the shift would be marked attended. The
// distance check is only a control if the browser cannot reach around it.

async function clockApi(body) {
  ...
  if (!res.ok) {
    const err = new Error(json.error || 'Could not record attendance.');
    err.verdict  = json.verdict || null;
    err.geofence = json.geofence || null;
...
/** @param {{lat:number, lng:number, accuracyM:number, sampleCount?:number}} fix */
export async function clockInHca(fix) {
  return clockApi({ action: 'in', ...fix });
}

export async function clockOutHca(fix) {
  return clockApi({ action: 'out', ...fix });
}
...
export async function getAllMapEntities() {
  ...
  for (const c of clients) {
    ...
    for (const p of (c.patients || [])) {
      // A patient's own care address if one is pinned, otherwise the client's
      // premises — flagged as inherited, because that distinction decides
      // whether the attendance geofence is measuring the right house.
      const own = isCoord(p.careLat, p.careLng);
      entities.push({
        id: p.id, type: 'patient', label: p.name,
        sub: own ? (p.careAddress || `Patient of ${c.name}`) : `Patient of ${c.name} · using client pin`,
        lat: own ? Number(p.careLat) : (c.lat ?? null),
        lng: own ? Number(p.careLng) : (c.lng ?? null),
        color: '#d97706',
        parentClientId: c.id,
        inheritedPin: !own,
        geofenceRadiusM: p.geofenceRadiusM || null,
      });
    }
  }
...
/**
 * Pin one patient's care address.
 *
 * Dragging a patient marker used to call updateClientCoords, so it moved the
 * client row — and with it every other patient of that client. Two patients at
 * two addresses could never be pinned separately, which is exactly the case
 * the attendance geofence has to get right.
 */
export async function updatePatientCoords(clientId, patientId, lat, lng, extra = {}) {
  const patch = { careLat: lat, careLng: lng };
  if (extra.careAddress !== undefined)     patch.careAddress = extra.careAddress;
  if (extra.geofenceRadiusM !== undefined) patch.geofenceRadiusM = normaliseRadius(extra.geofenceRadiusM);
  return updatePatient(clientId, patientId, patch);
}
