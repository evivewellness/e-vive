...
export async function createHcaApplication(formData) {
  ...
  // Strip base64 file data before storing — keeps row small; metadata (name, type, size) retained
  const sanitizedRest = {
    ...rest,
    certifications: (rest.certifications || []).map(({ fileDataUrl, ...cert }) => cert),
  };
