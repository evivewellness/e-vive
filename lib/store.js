...
function contactMsgFromDb(r) {
  if (!r) return null;
  return {
    id: r.id, fname: r.fname, lname: r.lname, email: r.email, phone: r.phone,
...
export async function getAllHcaApplications() {
  const { data, error } = await supabase.from('hca_applications').select('*').order('applied_at', { ascending: false });
  if (error) throw new Error(error.message);
...
export async function updateHcaApplication(id, patch) {
  const map = {
    ...
    county:'county', plan:'plan', bio:'bio', formData:'form_data',
  };
  const db = {};
  for (const [k, v] of Object.entries(patch)) { if (map[k]) db[map[k]] = v; }
...
export async function getAllContactMessages() {
  ...
  return (data || []).map(contactMsgFromDb);
}
...
export async function createContactMessage(data) {
  ...
  const { data: row, error } = await supabase.from('contact_messages').insert({
    fname, lname, email, phone, subject, message, status: 'unread'
  }).select().single();
  if (error) throw new Error(error.message);
  await logActivity({ type: 'contact_message_received', email, subject });
  return contactMsgFromDb(row);
}
