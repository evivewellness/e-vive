...
export async function createHcaProfile(data) {
  ...
  const { data: row, error } = await supabase.from('hca_profiles').insert({
    ...
    // Applications have stored a scrypt hash since /api/applications/create
    // took over intake. Carry the algorithm flag so login verifies it as a
    // hash rather than comparing it as a literal password.
    password_algo: String(password || '').startsWith('scrypt$') ? 'scrypt' : 'plain',
